...
- **WebGPU Rendering**: Leverages modern WebGPU API for enhanced performance (50-100% FPS improvement) with automatic fallback to WebGL.
...
- **Performance Panel**: Real-time vertex count and file size display
- **Model Loading**: File upload and URL loading with progress feedback
- **File Size Tracking**: Automatic size detection for both local and remote files
- **Loading States**: Immediate feedback for shared URL loads with professional spinner
- **Debug Information**: Device detection, system capabilities, and current WebGPU/WebGL renderer status
